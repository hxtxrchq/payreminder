...
class SaveDebtUseCase {
  ...
  Future<SaveDebtResult> save({
    ...
    if (type == DebtType.punctual) {
      ...
      for (final o in uniqueOffsets) {
        await NotificationService.instance.scheduleOccurrenceReminder(
  ...
  List<DateTime> _generateRecurrenceDates(
    ...
    var current = DateTime(
        rec.fechaInicio.year, rec.fechaInicio.month, rec.fechaInicio.day);
    var generated = 0;
    while (current.isBefore(endDate) && generated < maxOccurrences) {
      dates.add(current);
      generated++;
